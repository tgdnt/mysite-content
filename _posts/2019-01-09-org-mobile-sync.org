...
<aside>
#+END_HTML
*Note:* In the code examples below, it's assumed that the local /Org Mobile/ directory is ~~/mobileorg~ and that ~rclone~ is configured with a cloud service named ~Dropbox~, within which the /Org Mobile/ directory is ~Apps/MobileOrg~.
...
Please share your thought and ideas on this, or if you have seen any code that can fill a hole here.
